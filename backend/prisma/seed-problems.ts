...
} from "../src/modules/judge/supported-languages";
import { PROBLEM_CATALOG, type ProblemSeed } from "./problem-catalog";
...
async function clearProblemContent(problemId: string) {
  const testcaseIds = (
...
async function seedProblem(seed: ProblemSeed) {
  ...
  const invalidIds = publishedProblems
    .filter((problem) => {
      ...
      const validTemplates =
        problem.codeTemplates.length > 0 &&
        problem.codeTemplates.every(
          (template) =>
            isSupportedJudgeLanguage(template.language) &&
