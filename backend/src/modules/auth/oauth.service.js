...
async function exchangeCodeForToken(provider, code) {
    ...
    if (!response.ok || data.error || !data.access_token) {
        throw new AppError(400, "OAUTH_TOKEN_EXCHANGE_FAILED", data.error_description ?? data.error ?? "OAuth token exchange failed.");
    }
    return data;
}
...
async function findOrCreateOAuthUser(profile) {
    ...
    const usernameSource = profile.username ?? profile.email?.split("@")[0] ?? profile.name ?? "user";
...
export const oauthService = {
    createAuthorizationUrl(provider, query) {
        const config = getOAuthProviderConfig(provider);
