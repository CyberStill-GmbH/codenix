...
function mapTags(problem: AdminProblemListModel | AdminProblemDetailModel) {
  return problem.topics.map((item) => item.topic.name);
}

function splitConstraints(constraints: string) {
  return constraints
    .split("\n")
    .map((item) => item.trim())
    .filter(Boolean);
...
function buildStarterCode(problem: AdminProblemDetailModel) {
...
export function toAdminProblemListItem(problem: AdminProblemListModel) {
  return {
    id: problem.id,
    title: problem.title,
    slug: problem.slug,
...
export function toAdminProblemDetails(problem: AdminProblemDetailModel) {
  const supportedLanguages = problem.codeTemplates.map(
