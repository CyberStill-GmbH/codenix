...
import { isSupportedJudgeLanguage, SUPPORTED_JUDGE_LANGUAGES } from "../../judge/supported-languages";
import { toAdminLegacyTestcaseItem, toAdminProblemDetails, toAdminProblemListItem } from "./admin-problems.mapper";
...
async function upsertTopic(name) {
...
function buildCodeTemplates(input) {
    return input.supported_languages.map((language) => ({
        language,
        starterCode: input.starter_code[language] ?? ""
    }));
}
function validateSupportedLanguages(input) {
    const unsupportedLanguages = input.supported_languages.filter((language) => !isSupportedJudgeLanguage(language));
    if (unsupportedLanguages.length === 0)
        return;
    throw new AppError(422, "UNSUPPORTED_LANGUAGE", "One or more languages do not have an available judge runner.", {
...
function buildTestcases(input) {
    return input.testcases.map((testcase, index) => {
        const visibility = testcase.is_sample
            ? "sample"
            : "hidden";
        return {
...
async function buildProblemTopics(input) {
...
async function findProblemOrThrow(identifier) {
...
async function findAdminProblemDetails(identifier) {
    const problem = await prisma.problem.findFirst({
        where: {
            OR: [
                {
...
async function validatePublishable(problemId) {
...
async function getNextTestcaseOrderIndex(problemId) {
...
async function findTestcaseOrThrow(problemId, testcaseId) {
...
export const adminProblemsService = {
    ...
    async listTestcases(identifier) {
        const problem = await findProblemOrThrow(identifier);
    ...
    async createTestcase(identifier, input) {
        const problem = await findProblemOrThrow(identifier);
    ...
    async updateTestcase(identifier, testcaseId, input) {
        const problem = await findProblemOrThrow(identifier);
    ...
    async deleteTestcase(identifier, testcaseId) {
        const problem = await findProblemOrThrow(identifier);
        const testcase = await findTestcaseOrThrow(problem.id, testcaseId);
...
//# sourceMappingURL=admin-problems.service.js.map