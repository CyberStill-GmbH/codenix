...
exit "$exit_code"
`.trim();
export function wrapCommandWithMetrics(command) {
    return ["sh", "-c", CGROUP_MEMORY_SCRIPT, "codenix-metrics", ...command];
}
export function extractMemoryMetric(stderr) {
...
export function buildDockerArgs(options, containerName) {
    const args = [
        "run",
        "--name",
        containerName,
    ...
    if (options.readOnlyRootfs) {
        args.push("--read-only", "--tmpfs", "/tmp:rw,noexec,nosuid,nodev,size=50m,mode=1777");
    }
    if (options.user)
    ...
    args.push("-w", options.workdir, options.image, ...wrapCommandWithMetrics(options.command));
    return args;
}
export async function runDockerContainer(options) {
