...
import {
  buildDockerArgs,
  extractMemoryMetric,
  wrapCommandWithMetrics,
} from "../modules/judge/sandbox/docker-runner";
import { Workspace } from "../modules/judge/sandbox/workspace";
import type {
  Sandbox,
  SandboxExecutionResult,
  SandboxRunOptions,
} from "../modules/judge/sandbox/types";
...
const baseExecutionResult: SandboxExecutionResult = {
  stdout: "",
  stderr: "",
  executionTimeMs: 7,
  memoryKb: 1234,
...
class RecordingSandbox implements Sandbox {
  readonly calls: SandboxRunOptions[] = [];

  constructor(private readonly results: SandboxExecutionResult[] = []) {}

  async run(options: SandboxRunOptions): Promise<SandboxExecutionResult> {
...
describe("judge sandbox metrics", () => {
  it("extracts peak memory in KB and removes the internal marker from stderr", () => {
    const result = extractMemoryMetric(
...
describe("judge workspace", () => {
  it("owns temporary file creation, mounts, and cleanup", async () => {
    ...
    await expect(
      fs.readFile(workspace.filePath("solution.py"), "utf8"),
...
describe("judge runners sandbox abstraction", () => {
  it("Python runner delegates execution metrics to the injected sandbox", async () => {
    ...
    const runner = new PythonRunner(
      { sourceCode: "print(42)", timeLimitMs: 1000, memoryLimitMb: 128 },
      sandbox,
    );
  ...
  it("compiled runners report compilation errors from sandbox metrics", async () => {
    ...
    const runner = new CRunner(
      { sourceCode: "not c", timeLimitMs: 1000, memoryLimitMb: 128 },
      sandbox,
    );
  ...
  it("runner factory injects the same sandbox without exposing Docker to runners", async () => {
    ...
    const runner = createRunner(
      "javascript",
      { sourceCode: "console.log(1)", timeLimitMs: 1000, memoryLimitMb: 64 },
      sandbox,
    );
