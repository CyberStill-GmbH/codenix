...
async function createProblem(index) {
...
async function createSubmission(userId, problemId, result = "accepted") {
...
describe("Users API", () => {
    ...
    it("returns percentile and aggregate ranking distribution", async () => {
        ...
        expect(res.body.distribution.reduce((sum, item) => sum + item.count, 0)).toBe(res.body.totalUsers);
    });
});
