...
import { ArrowDown, ArrowUp, MessageCircle, Send, Sparkles } from 'lucide-react'
...
export function CommentsSection({ problemId }: CommentsSectionProps) {
  ...
  useEffect(() => {
    if (!problemId) return
    getComments(problemId, sort).then((response) => setComments(response.data)).catch(() => setError('No se pudieron cargar los comentarios.')).finally(() => setIsLoading(false))
  }, [problemId, sort])
