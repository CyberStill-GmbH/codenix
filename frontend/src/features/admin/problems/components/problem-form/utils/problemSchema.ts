...
export type ProblemFormErrors = Partial<
  Record<keyof AdminProblemFormValues, string>
>;
...
const problemFormBaseSchema = z.object({
  ...
  descriptionMarkdown: z
    .string()
    .trim()
    .min(1, "La descripcion Markdown es requerida."),
...
export const problemFormSchema = problemFormBaseSchema.superRefine(
  (values, context) => {
